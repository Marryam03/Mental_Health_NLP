"""Generate Gemini Phase 1 notebook with batch CSV processing."""
import json
...
# Build the prompt template as separate strings to avoid nesting issues
PROMPT_HEADER = (
    "# Instruction\n"
...
def generate_gemini():
    cells = [
        ...
        code_cell(
            ...
            'INPUT_CSV  = "Dataset/mental_health.csv"\n'
            'OUTPUT_CSV = "Phase1_Prompt Engineering/gemini_labels_output.csv"',
            metadata={"id": "ibIpZe9h_Fq5"}
        ),
        code_cell(
            "RPM_LIMIT   = 28    # Gemini 1.5 Flash free tier: 1,500 req/day\n"
            "WINDOW_SEC  = 60\n"
            "REST_SEC    = 35\n"
            "CHUNK_SIZE  = 20    # rows per API call (tune based on average text length)",
        ...
        code_cell(build_core_code("gemini"), metadata={"id": "pjegrEDR_OiJ"}),
        code_cell(
    ...
    with open("Phase1_Prompt Engineering/Gemini_Prompt_Labeling.ipynb", "w", encoding="utf-8") as f:
        json.dump(nb, f, indent=2, ensure_ascii=False)
    print("Generated Gemini notebook")
